...
- 剪贴板通过 macOS 系统剪贴板轮询自动捕获文本、图片和文件（文件仅存路径，不复制内容），历史持久化到本地 SQLite，支持搜索、类型筛选、收藏、源文件失效检测，以及将历史文本、图片或文件重新放回系统剪贴板使用；剪贴板入口会打开偏宽的独立轻量窗口并默认靠屏幕下方展示，设置中可在横向卡片和纵向列表两种样式之间切换，卡片可收藏但不改变时间排序，可通过「收藏」筛选单独查看；点击卡片会直接使用当前内容并关闭弹窗，窗口失焦或点击透明空白区域也会按自动关闭设置关闭，底部仅保留收藏和删除；历史条数上限会立即裁剪未收藏旧记录，收藏项不受上限影响，「清空未收藏」也会保留收藏项及对应图片缓存
- 剪贴板类型色在浅色和深色主题下分别保持语义一致：文本为浅蓝灰、图片为浅绿、文件为浅琥珀、未知类型为浅中性灰，横向卡片 header 使用浅色背景和同色深字，纵向列表使用同语义色的左侧细边线
- 剪贴板搜索框有内容时在输入框右侧显示清除图标，卡片收藏和删除使用带提示的图标按钮；横向卡片底部左侧展示文本长度或文件、图片大小，操作工具固定在右侧，横向窗口打开后贴合当前显示器可用工作区底边。横向布局可在设置中选择半屏、默认、铺满屏幕三档窗口宽度和小、默认、大三档卡片尺寸；纵向窗口默认宽度为 `620px`，可选择 `560 / 720 / 880px` 三档高度。三组尺寸设置都使用同一套小卡片选择器，卡片内同时展示档位和实际尺寸。纵向列表在组件内部滚动，整张卡片点击即使用，类型通过左侧 `2px` 浅色语义边线区分；快捷键打开时会先读取已保存的布局和尺寸，完成窗口调整后再显示，避免纵向模式先闪现横向布局
